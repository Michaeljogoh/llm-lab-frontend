...
function SheetOverlay({
  ...
  return (
    <SheetPrimitive.Overlay
      className={cn(
        "fixed inset-0 z-50 bg-black/45 overlay-enter",
        className
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        className={cn(
          "fixed z-50 flex flex-col bg-background shadow-xl transition-transform duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] dark:border-white/8 dark:shadow-[0_0_80px_rgb(0_0_0_/_60%)]",
          side === "right" &&
            "inset-y-0 right-0 h-full w-full border-l sm:max-w-xl sheet-enter",
          side === "left" &&
